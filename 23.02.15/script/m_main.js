...
//////////////////메인슬라이드////////////////
  ...
  let img_w = mleft * 2;
  function moveLeft() {
    $(".prd_txt").css("margin-left", mleft + "%");
    mleft -= 1;

    if (mleft <= -20) {
      mleft = 0;
      $(".prd_txt ul").appendTo($(".prd_txt"));
    }
  }

  setInterval(moveLeft, 100);

  let news_li =$('news_tab_li > li')
  ...
  $('.prd_img img:not(.image1)').fadeOut(300);
  
  // 3초마다 이미지 변경
  setInterval(() => {
    // 현재 이미지 숨기기
    $(`.image${currentImgIndex+1}`).fadeOut(300);
    
    // 다음 이미지 보여주기
    currentImgIndex = (currentImgIndex + 1) % images.length;
    $(`.image${currentImgIndex+1}`).fadeIn(300);
  }, 3000);
  });
